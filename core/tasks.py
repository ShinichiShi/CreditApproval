...
def import_customer_data():
    try:
        ...
        required_columns = ['First Name', 'Last Name', 'Age', 'Phone Number', 'Monthly Salary', 'Approved Limit']
        ...
        logger.info(f"Customer import completed. Created: {created_count}, Errors: {error_count}")
        return {
            "success": True, 
...
def import_loan_data():
    try:
        ...
        column_mapping = {
            ...
            'Monthly payment': ['Monthly payment', 'Monthly Payment', 'monthly_payment', 'MonthlyPayment'],
            'EMIs paid on Time': ['EMIs paid on Time', 'EMIs_paid_on_Time', 'emis_paid_on_time'],
            'Date of Approval': ['Date of Approval', 'date_of_approval', 'DateOfApproval'],
        ...
        for standard_name, variations in column_mapping.items():
            ...
            if standard_name not in actual_columns:
                logger.error(f"Column '{standard_name}' not found. Available columns: {list(df.columns)}")
                return {'status': 'error', 'message': f'Column {standard_name} not found'}
            ...
            for index, row in df.iterrows():
                ...
                try:
                    ...
                    Loan.objects.create(
                        ...
                        start_date=pd.to_datetime(row[actual_columns['Date of Approval']]).date(),
...
def check_data_status():
    ...
    try:
        ...
        logger.info(f"Database status - Customers: {customer_count}, Loans: {loan_count}")
